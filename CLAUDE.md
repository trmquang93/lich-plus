...
1. **In Swift code**, use the `String(localized:)` initializer with raw English text:
   ```swift
   // Always use raw English strings - NO dot-notation
   Text(String(localized: "Lunar Special Dates"))
   Label(String(localized: "Add Event"), systemImage: "plus")
   ```

2. **Build with Xcode** to auto-extract localization keys:
   - When you build the project, Xcode automatically adds new `String(localized:)` usages to `Localizable.xcstrings`
   - The English text becomes both the key and the default English translation
   - Open `Localizable.xcstrings` in Xcode after building to see the new entries

3. **Add Vietnamese translations** (separate task):
   - Open `Localizable.xcstrings` in Xcode
   - Find the auto-extracted key (uses the raw English string)
   - Add the Vietnamese translation in the "vi" localization
   - This is typically done as a separate task after feature implementation

4. **String catalog structure** (auto-generated by Xcode):
   ```json
   {
     "Lunar Special Dates" : {
...
**Important Rules:**
- **NEVER use dot-notation** like `"settings.lunarSpecialDates.title"` or `"calendar.saveEvent"`
- **ALWAYS use raw English strings** like `"Lunar Special Dates"` or `"Save Event"`
- The English text serves as both the key and the default translation
- Build with Xcode to auto-extract keys to `Localizable.xcstrings`
- Vietnamese translation is a separate task done after feature implementation
